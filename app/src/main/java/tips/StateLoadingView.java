...
public class StateLoadingView extends View {
    ...
    public StateLoadingView(Context context, AttributeSet attrs) {
        this(context, attrs, R.attr.NGWLoadingStyle);
    }

    public StateLoadingView(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        TypedArray array = getContext().obtainStyledAttributes(attrs, R.styleable.StateLoadingView, defStyleAttr, 0);
        mSize = array.getDimensionPixelSize(R.styleable.StateLoadingView_ngw_loading_view_size, DensityUtils.dip2px(context,40));
        mPaintColor = array.getInt(R.styleable.StateLoadingView_android_color, Color.WHITE);
        progressStyle = array.getInt(R.styleable.StateLoadingView_ngw_loading_progress_style, PROGRESS);
        array.recycle();
        initPaint();
    }

    public StateLoadingView(Context context, int size, int color) {
        super(context);
        mSize = size;
        mPaintColor = color;
        initPaint();
